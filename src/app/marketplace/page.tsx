      ...
      <div style={{ background: 'var(--ink)', border: '1px solid var(--border-dark)', width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ padding: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {error && <p style={{ fontFamily: 'var(--mono)', fontSize: '0.62rem', color: '#f87171', letterSpacing: '0.04em', padding: '0.65rem 1rem', border: '1px solid rgba(248,113,113,0.3)', background: 'rgba(248,113,113,0.06)' }}>{error}</p>}
            ...
            <p style={{ fontFamily: 'var(--mono)', fontSize: '0.58rem', color: 'rgba(247,245,240,0.2)', textAlign: 'center', letterSpacing: '0.06em', lineHeight: 1.6 }}>Secure checkout via Stripe. Cancel anytime.</p>
          </div>
        </div>
      </div>
    </div>
...
export default function Marketplace() {
  ...
  useEffect(() => {
    ...
    if (params.get('canceled') === 'true') { setCanceled(true); setShowWaitlist(true); window.history.replaceState({}, '', '/marketplace') }
  }, [])

  useEffect(() => {
    const fetchExperts = async () => {
      try {
        const { data, error } = await supabase.from('operators').select('id, name, handle, title, location, avatar, tags, specialty, rate, rate_type, rating, reviews, available, featured, bio, deliverables, tier, mux_playback_id, r2_key, youtube_url').eq('approved', true).order('featured', { ascending: false }).order('rating', { ascending: false })
        if (error) throw error
        if (data) setExperts(data.map((op, i) => ({ ...op, avatarColor: AVATAR_COLORS[op.avatar] || COLOR_POOL[i % COLOR_POOL.length], tier: op.tier || 'pro', tags: op.tags || [], deliverables: op.deliverables || [], mux_playback_id: op.mux_playback_id || '', r2_key: op.r2_key || '', youtube_url: op.youtube_url || '' })))
  ...
  const filtered = experts.filter(op => {
    ...
    const matchSearch = !search || op.name.toLowerCase().includes(search.toLowerCase()) || (op.specialty || '').toLowerCase().includes(search.toLowerCase()) || op.tags.some((t: string) => t.toLowerCase().includes(search.toLowerCase()))
  ...
  return (
    <div style={{ background: 'var(--ink)', minHeight: '100vh', fontFamily: 'var(--sans)', paddingTop: '60px' }}>
      {showWaitlist && <WaitlistModal onClose={() => { setShowWaitlist(false); setCanceled(false) }} />}

      <Navbar />

      {canceled && !showWaitlist && (
        <div style={{ background: 'rgba(248,113,113,0.1)', borderBottom: '1px solid rgba(248,113,113,0.2)', padding: '0.75rem 1.5rem', textAlign: 'center', fontFamily: 'var(--mono)', fontSize: '0.65rem', color: '#f87171', letterSpacing: '0.08em' }}>
          Payment canceled. Your application was not submitted.
        </div>
      )}
        ...
        <div className="hero-grid" style={{ position: 'relative', zIndex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }}>
          <div>
            ...
            <h1 style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(2.5rem, 5vw, 4.5rem)', fontWeight: 400, lineHeight: 1.05, marginBottom: '1.25rem' }}>
              Find your<br /><em style={{ fontStyle: 'italic', color: 'var(--coral)' }}>AI expert.</em>
            </h1>
            ...
            <button onClick={() => setShowWaitlist(true)} style={{ fontFamily: 'var(--sans)', fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', background: 'transparent', color: 'rgba(247,245,240,0.5)', border: '1px solid var(--border-dark)', padding: '0.75rem 1.5rem', cursor: 'pointer', transition: 'all 0.15s' }} onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--coral-border)'; e.currentTarget.style.color = 'var(--page)' }} onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border-dark)'; e.currentTarget.style.color = 'rgba(247,245,240,0.5)' }}>Join as an expert</button>
          </div>
      ...
      <div style={{ position: 'sticky', top: 60, zIndex: 90, background: 'rgba(15,15,14,0.97)', borderBottom: '1px solid var(--border-dark)', backdropFilter: 'blur(12px)', padding: '0.75rem 1rem' }}>
        <div style={{ position: 'relative', marginBottom: '0.65rem' }}>
          ...
          <input type="text" placeholder="Search by skill, tool, or specialty..." value={search} onChange={e => setSearch(e.target.value)} style={{ width: '100%', background: 'var(--ink-2)', border: '1px solid var(--border-dark)', color: 'var(--page)', fontFamily: 'var(--sans)', fontSize: '0.85rem', padding: '0.65rem 0.85rem 0.65rem 4rem', outline: 'none', transition: 'border-color 0.2s', boxSizing: 'border-box' }} onFocus={e => e.target.style.borderColor = 'var(--coral-border)'} onBlur={e => e.target.style.borderColor = 'var(--border-dark)'} />
        </div>
        <div style={{ display: 'flex', gap: '0.4rem', overflowX: 'auto', paddingBottom: '2px', scrollbarWidth: 'none' }}>
          {specialties.map(s => <button key={s} onClick={() => setActiveFilter(s)} style={{ fontFamily: 'var(--mono)', fontSize: '0.6rem', fontWeight: 300, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '0.38rem 0.75rem', whiteSpace: 'nowrap', border: '1px solid', borderColor: activeFilter === s ? 'var(--coral)' : 'var(--border-dark)', background: activeFilter === s ? 'var(--coral)' : 'transparent', color: activeFilter === s ? '#fff' : 'rgba(247,245,240,0.4)', cursor: 'pointer', transition: 'all 0.15s', flexShrink: 0 }}>{s}</button>)}
          <button onClick={() => setAvailableOnly(!availableOnly)} style={{ fontFamily: 'var(--mono)', fontSize: '0.6rem', fontWeight: 300, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '0.38rem 0.75rem', whiteSpace: 'nowrap', border: '1px solid', borderColor: availableOnly ? '#3ecf8e' : 'var(--border-dark)', background: availableOnly ? 'rgba(62,207,142,0.12)' : 'transparent', color: availableOnly ? '#3ecf8e' : 'rgba(247,245,240,0.4)', cursor: 'pointer', transition: 'all 0.15s', flexShrink: 0, marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: availableOnly ? '#3ecf8e' : 'rgba(247,245,240,0.2)' }} />Available
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: '1.25rem 1rem', maxWidth: 900, margin: '0 auto' }}>
        ...
        {loading && <div style={{ display: 'flex', flexDirection: 'column', gap: '1px' }}>{[1,2,3].map(i => <div key={i} style={{ height: 80, background: 'var(--ink-2)', border: '1px solid var(--border-dark)', opacity: 0.5 }} />)}</div>}
        {!loading && filtered.length === 0 && <div style={{ textAlign: 'center', padding: '5rem 1.25rem', fontFamily: 'var(--serif)', fontSize: '1.4rem', color: 'rgba(247,245,240,0.25)' }}>No experts match that search.</div>}
      ...
      <div style={{ margin: '2rem 1rem 4rem', padding: 'clamp(2rem, 6vw, 3.5rem) clamp(1.25rem, 5vw, 3rem)', position: 'relative', overflow: 'hidden', maxWidth: 900, border: '1px solid var(--border-dark)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <h2 style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(1.5rem, 4vw, 2.5rem)', fontWeight: 400, lineHeight: 1.15, marginBottom: '0.85rem' }}>Get listed.<br /><em style={{ fontStyle: 'italic', color: 'var(--coral)' }}>Get found. Get hired.</em></h2>
          <p style={{ fontFamily: 'var(--sans)', fontSize: '0.85rem', color: 'rgba(247,245,240,0.4)', lineHeight: 1.7, maxWidth: '44ch', marginBottom: '1.5rem' }}>Plans from $29/mo. Approved experts get listed, leads routed, and full dashboard access.</p>

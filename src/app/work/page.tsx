  ...
  return (
    <div style={{ background: 'var(--ink)', minHeight: '100vh', fontFamily: 'var(--sans)', paddingTop: '60px' }}>
      ...
      <div style={{ padding: 'clamp(4rem, 10vw, 8rem) clamp(1rem, 4vw, 3rem) clamp(2rem, 5vw, 4rem)', borderBottom: '1px solid var(--border-dark)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '-30%', right: '-5%', width: '35vw', height: '35vw', borderRadius: '50%', background: 'radial-gradient(ellipse, rgba(232,82,26,0.07) 0%, transparent 65%)', pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'absolute', bottom: '-0.5rem', right: 'clamp(1rem, 4vw, 3rem)', fontFamily: 'var(--serif)', fontSize: 'clamp(6rem, 18vw, 14rem)', fontWeight: 400, color: 'rgba(247,245,240,0.03)', lineHeight: 1, pointerEvents: 'none', userSelect: 'none' }}>
          {projects.length}
        </div>
      </div>
          ...
          >
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
                <h2 style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(1.5rem, 3vw, 2.2rem)', fontWeight: 400, color: hovered === p.id ? 'var(--coral)' : 'var(--page)', lineHeight: 1.1, transition: 'color 0.2s' }}>{p.name}</h2>
                <span style={{ fontFamily: 'var(--mono)', fontSize: '0.58rem', fontWeight: 300, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--coral)', border: '1px solid var(--coral-border)', padding: '0.15rem 0.5rem' }}>{p.tag}</span>
              </div>
            ...
            <div style={{ transform: hovered === p.id ? 'translateY(-4px) scale(1.01)' : 'translateY(0) scale(1)', transition: 'transform 0.3s cubic-bezier(0.22, 1, 0.36, 1)' }}>
              <div style={{ borderRadius: '10px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)', boxShadow: hovered === p.id ? '0 24px 64px rgba(0,0,0,0.6), 0 0 0 1px rgba(232,82,26,0.2)' : '0 8px 32px rgba(0,0,0,0.4)', transition: 'box-shadow 0.3s ease' }}>
            ...
            <div style={{ position: 'absolute', top: 'clamp(1.5rem, 4vw, 2.5rem)', right: 0, fontFamily: 'var(--mono)', fontSize: '0.75rem', color: hovered === p.id ? 'var(--coral)' : 'rgba(247,245,240,0.15)', transition: 'color 0.2s, transform 0.2s', transform: hovered === p.id ? 'translate(4px, -2px)' : 'translate(0,0)' }}>↗</div>
          </a>
        ))}
      </div>
          ...
          <a key={p.id} href={p.href} target="_blank" rel="noopener noreferrer" style={{ display: 'block', textDecoration: 'none', border: '1px solid var(--border-dark)', borderRadius: '12px', overflow: 'hidden', background: 'var(--ink-2)' }}>
            ...
            <div style={{ padding: '1.1rem 1.1rem 1.25rem' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '0.5rem', marginBottom: '0.5rem' }}>
                ...
                <span style={{ fontFamily: 'var(--mono)', fontSize: '0.5rem', fontWeight: 300, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--coral)', border: '1px solid var(--coral-border)', padding: '0.15rem 0.45rem', whiteSpace: 'nowrap', flexShrink: 0, marginTop: '0.2rem' }}>{p.tag}</span>
              </div>
      ...
      {/* FOOTER CTA */}
      <div style={{ borderTop: '1px solid var(--border-dark)', padding: 'clamp(3rem, 8vw, 6rem) clamp(1rem, 4vw, 3rem)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
